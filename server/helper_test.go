...
func newTestProtocolManager(mode downloader.SyncMode, blocks int, generator func(int, *bc.BlockGen), newtx chan<- []*types.Transaction) (*ProtocolManager, *database.MemDatabase, error) {
	var (
		...
		blockchain, _ = bc.NewBlockChain(db,engine,params.TestChainConfig)
	)
	chain := bc.GenerateChain(config, genesis, engine, db, blocks, generator)
	if _, err := blockchain.InsertChain(chain); err != nil {
		panic(err)
	}

	pm, err := NewProtocolManager(config, mode, DefaultConfig.NetworkId, evmux, &testTxPool{added: newtx}, engine, blockchain, db)
	if err != nil {
		return nil, nil, err
	}
	pm.Start(1000)
